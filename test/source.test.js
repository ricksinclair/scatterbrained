...
describe('pickPrimarySource', () => {
  ...
  it('prefers the source most relevant to the node name over an earlier generic one', () => {
    const edges = [
      ...
      { dir: 'in', label: 'Source', name: 'usfwc-profile.md', file_path: '/wc/usfwc-profile.md' },
    ];
    const r = pickPrimarySource({ name: 'U.S. Federation of Worker Cooperatives (USFWC)' }, edges);
    expect(r.filePath).toBe('/wc/usfwc-profile.md');   // matches 'usfwc', not the first (colorado)
  });
