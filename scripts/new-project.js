...
function hookScript(eventName, additionalContext) {
  ...
  return `#!/bin/sh
# Engram graph-first hook (${eventName}) — generated by new-project. Edit the text
...
export function agentConfigFiles(name) {
  const sessionText =
    `[Engram graph-first rule — project: ${name}] The Engram knowledge graph is the PRIMARY ` +
  ...
  const promptText =
    `[graph-first] This project's work/decisions/rationale/history/status → consult the Engram ` +
    `graph FIRST (npm run resume / search, or neo4j MCP), before repo/web.`;
  const cmd = (f) => `sh "$CLAUDE_PROJECT_DIR/.claude/hooks/${f}"`;
  const settings = {
    hooks: {
      SessionStart: [{ hooks: [{ type: 'command', command: cmd('engram-graph-first-session.sh'), timeout: 5, statusMessage: 'Loading Engram graph-first rule' }] }],
      UserPromptSubmit: [{ hooks: [{ type: 'command', command: cmd('engram-graph-first-prompt.sh'), timeout: 5 }] }],
    },
  };
...
export function writeAgentConfig(repoDir, name) {
  ...
  fs.writeFileSync(path.join(hooksDir, 'engram-graph-first-session.sh'), sessionHook);
  fs.writeFileSync(path.join(hooksDir, 'engram-graph-first-prompt.sh'), promptHook);
