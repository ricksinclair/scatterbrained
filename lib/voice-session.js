...
// to escalate toward this after consecutive timeouts. (Rick, 2026-07-04: keep the
...
export function createVoiceSession({ now = Date.now, setTimer = setTimeout, clearTimer = clearTimeout, onEvent = () => {} } = {}) {
  ...
  return {
    ...
    utterance({ text, source = 'text', interrupted = false, ui = null } = {}) {
      ...
      if (pendingListen) settleListen(u);
    ...
    say(sessionId, { text } = {}) {
      ...
      return new Promise((resolve) => {
        ...
        onEvent('say', { msg_id: msgId, text: t, model: agent.model });
      });
    },
    ...
    // ── provenance breadcrumbs (consumed by Phase 6 capture) ────────────────────
    touch(nodeId, meta = {}) { if (nodeId) touched.set(String(nodeId), { name: meta.name || null, label: meta.label || null }); seen(); },
