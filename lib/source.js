...
// Resolve a leading `~` / `~/` against the home dir. Pure, lexical, no I/O.
//
// A Source's file_path is written by hand (agents MERGE them) as often as by
// document-index.js, and hands type `~/Projects/...`. But path.isAbsolute('~/x') is FALSE,
// so any caller doing "not absolute → resolve against the repo" turns a tilde path into
// <repo>/~/Projects/... — a path that cannot exist. Both the roots allowlist and a Source's
// own file_path need this, so it lives in one place.
//
// `~user` is deliberately NOT expanded: that names another account's home, not this one.
export function expandHome(p, homedir = '') {
  const s = String(p ?? '');
  if (s === '~') return homedir;
  if (!s.startsWith('~/')) return s;              // '' , '/abs', 'rel', '~alice' all pass through
...
export function expandRoots(config = {}, homedir = '') {
  return (config.roots || [])
    ...
    .filter(Boolean)
    .map((p) => expandHome(p, homedir))
    .map((p) => path.resolve(p));
}

// True only if `target` resolves to a location inside one of `roots` — blocks `..` traversal
// (lexically) and prefix-collision false-positives ('/a/bc' is NOT inside '/a/b'). Empty/absent
// target → false. NOTE: path.resolve is LEXICAL — it does NOT follow symlinks. The caller must
// pass an fs.realpathSync'd target (and realpath'd roots) to also block symlink escapes; the
// server does this via realOf() at every read endpoint. Kept fs-free here so it stays pure/tested.
...
// *about* it, not just the first readable one (e.g. an org node was showing a
// random region file instead of its own profile).
