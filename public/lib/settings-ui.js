...
// deps: { esc, THEMES, THEME_ORDER, applyTheme, applyAnim, setCalm, getTheme, openPerms }
//   getTheme(): { curTheme, curMode, calm, curAnim } — re-read on each render so it reflects changes.
// returns { open, close }.
export function initSettings({ esc, THEMES, THEME_ORDER, applyTheme, applyAnim, setCalm, getTheme, openPerms }) {
  ...
  function render(st) {
    const { curTheme, curMode, calm, curAnim } = getTheme();
  ...
  document.getElementById('set-settings').onclick = open;
  document.getElementById('settings-x').onclick = close;
  SETTINGS.addEventListener('click', (e) => { if (e.target === SETTINGS) close(); });
  window.addEventListener('keydown', (e) => { if (e.key === 'Escape' && !SETTINGS.hidden) close(); });
  return { open, close };
}
